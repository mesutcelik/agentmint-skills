...
description: Route Hermes `delegate_task(background=True)` to named, persistent AgentMint subagents — specialists that accumulate `/workspace/MEMORY.md` across calls. Default routing via `default_agent_name`, or per-call routing via an `agentmint-<name>` entry the LLM passes in `toolsets`. Optional `agentmint_delegate` plugin tool exposes a cleaner per-call surface. Polling delivery (no public HTTPS required); pay via Stripe-Link (link-cli).
version: 0.7.0
author: AgentMint
license: MIT
...
Bridges Hermes' `delegate_task(background=True)` to **named, persistent AgentMint subagents** — sandboxed Upstash Box runs whose `/workspace/MEMORY.md` accumulates context across every dispatch. Hibernate to zero between calls; billed only when running.

## Runtime usage (for the LLM)

After the operator wires the skill, Hermes' built-in `delegate_task` and (optionally) a new `agentmint_delegate` tool behave as follows. The LLM uses one of these patterns to dispatch:

### Default routing (operator passed `default_agent_name="..."`)

Every `delegate_task(background=True)` lands in ONE pre-named subagent. Its `/workspace/MEMORY.md` accumulates across calls — the subagent LEARNS.
...
### Per-call routing via `toolsets` (workaround for Hermes' missing dispatcher arg)

When the LLM wants to address a SPECIFIC named subagent per call (one specialist per repo, per customer, per domain), it embeds the target in `toolsets`:
...
The adapter parses `agentmint-pr-reviewer-myrepo`, routes to that subagent (overriding any `default_agent_name`), and strips the entry from the toolsets before composing the prompt. Each subagent maintains its own `/workspace/MEMORY.md`.

This is a **deliberate hack** to work around Hermes' `delegate_task` not having a dispatcher-target parameter. An upstream proposal has been filed; when it lands the convention will be replaced with a first-class arg.

### Optional `agentmint_delegate` plugin tool (cleanest per-call surface)

If operator pip-installed `agentmint-hermes-runner` AND called `set_dispatcher(...)`, Hermes auto-discovers a new tool alongside `delegate_task`:
...
LLM picks the subagent per call. Cleaner than the toolset hack because `agent_name` is a first-class arg. Use this when the operator has enabled it.
...
- A task naturally **fans out** into N independent slices and each slice should land in its own named specialist → pre-mint per slice and use either per-call routing (toolset hack) or `agentmint_delegate`.
...
**Default routing (one specialist that remembers everything):**
...
Every `delegate_task(background=True)` lands in `default-worker`. Cost: $0.10 one-time mint + ~$0.05 per call.

**Per-call routing via the `toolsets` hack (LLM picks among multiple named specialists):**

Pre-mint each specialist:

```bash
for NAME in pr-reviewer-myrepo support-acme oncall-payments; do
  curl -X POST https://api.agentmint.store/a2a \
    -H "Authorization: Bearer $AGENTMINT_JWT" \
    -H 'Content-Type: application/json' \
    -d "{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"agent.create\",\"params\":{
          \"name\":\"$NAME\",\"harness\":\"opencode\",\"model\":\"openrouter/fusion\"}}"
done
```

Wire the adapter — `default_agent_name` is optional (used as fallback when no `agentmint-*` is in toolsets):

```python
install_delegate_task_wrapper(dispatcher)   # toolset routing enabled by default
```

The LLM then dispatches per call:
...
The adapter parses `agentmint-pr-reviewer-myrepo`, routes there, strips the entry from toolsets. Cost: $0.10 per pre-mint + ~$0.05 per call.

**Plugin tool (cleanest per-call surface):**

Pre-mint specialists (same as above). Then in Hermes gateway startup:
...
The `agentmint_delegate` tool auto-registers via Hermes' `hermes_agent.plugins` entry-point. LLM calls `agentmint_delegate(agent_name="X", goal="...", async_=True)`. Cleaner than the toolset hack because `agent_name` is a first-class arg. Cost: $0.10 per pre-mint + ~$0.05 per call.

You can combine the plugin tool with default-routing or toolset-routing — both `delegate_task` and `agentmint_delegate` coexist:
...
Even without the `agentmint_delegate` plugin, the Hermes LLM can manage subagents directly via `terminal` + curl. The `delegate_task` patch routes to `default_agent_name` OR whichever subagent is encoded in `toolsets` (`agentmint-<name>`); for arbitrary fleet operations beyond dispatch, use direct `/a2a` calls.
...
## Hermes `delegate_task` coverage (v0.7)
...
| `toolsets=["agentmint-<name>", ...]` | ✅ (workaround) | **Routing hack** — adapter parses `agentmint-<name>`, dispatches to that subagent, strips entry from toolsets. See "Per-call routing" above. Upstream proposal filed (`docs/hermes-feature-request.md`). |
...
| `background=True` (PR #40946) | ✅ | Two paths: default routing via `default_agent_name`, or per-call routing via `agentmint-<name>` in toolsets |
| `agentmint_delegate(agent_name=…, …)` | ✅ | Tool via Hermes' `hermes_agent.plugins` entry-point; cleaner per-call routing surface |
...
| `api_mode` / `model` / `provider` / `base_url` / `api_key` | n/a | These configure Hermes' subagent LLM provider — AgentMint subagents run against AgentMint's server-managed stored keys (provider + model chosen at `agent.create` time) |
...
| Fresh conversation per call ("subagents know nothing") | **inverted** | This inversion is the core AgentMint value prop — `/workspace/MEMORY.md` survives every dispatch to a named subagent, so a specialist accumulates knowledge across calls |
...
- **Per-call pricing is smoothed** ($0.01–$0.075 per call, same band as all-inclusive `agent.run`). The one-time mint cost ($0.10) amortizes across every subsequent call to that subagent.
...
For mode canaries, see `examples/{persistent,plugin}.py` in the [agentmint-hermes](https://github.com/mesutcelik/agentmint-hermes) repo. The upstream proposal for a proper dispatcher arg on `delegate_task` lives in `docs/hermes-feature-request.md` in the same repo.
